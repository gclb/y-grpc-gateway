...
Then `go get -u -v` the following packages:
    ...
    go get -u -v github.com/gclb/y-grpc-gateway/v2/protoc-gen-grpc-gateway
    go get -u -v github.com/gclb/y-grpc-gateway/v2/protoc-gen-openapiv2
...
To fix this you need to run the `go get -u -v` commands and look for all lines starting with `# cd .;`. Copy and paste these lines into your shell and change the clone destination directories.
    ...
    git clone https://github.com/gclb/y-grpc-gateway $(cygpath -u $GOPATH)/src/github.com/gclb/y-grpc-gateway
