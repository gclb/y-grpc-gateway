...
- https://grpc-ecosystem.github.io/y-grpc-gateway/
...
import (
    _ "github.com/gclb/y-grpc-gateway/v2/protoc-gen-grpc-gateway"
    _ "github.com/gclb/y-grpc-gateway/v2/protoc-gen-openapiv2"
...
```sh
$ go install \
    github.com/gclb/y-grpc-gateway/v2/protoc-gen-grpc-gateway \
    github.com/gclb/y-grpc-gateway/v2/protoc-gen-openapiv2 \
...
You may alternatively download the binaries from the [GitHub releases page](https://github.com/gclb/y-grpc-gateway/releases/latest).
...
2. Download the provenance file `attestation.intoto.jsonl` from the [GitHub releases page](https://github.com/gclb/y-grpc-gateway/releases/latest).
3. Run the verifier:
```shell
slsa-verifier -artifact-path <the-binary> -provenance attestation.intoto.jsonl -source github.com/gclb/y-grpc-gateway -tag <the-tag>
```
...
### 2. Generate gRPC stubs
   ...
   > [the boilerplate repo](https://github.com/johanbrandhorst/y-grpc-gateway-boilerplate).
...
### 4. Generate reverse-proxy using `protoc-gen-grpc-gateway`
  ...
  #### 2. With custom annotations
   ...
   [Check our documentation](https://grpc-ecosystem.github.io/y-grpc-gateway/docs/mapping/grpc_api_configuration/)
...
### 5. Write an entrypoint for the HTTP reverse-proxy server
   ...
   import (
     ...
     "github.com/gclb/y-grpc-gateway/v2/runtime"
...
### 6. (Optional) Generate OpenAPI definitions using `protoc-gen-openapiv2`
   ...
   `buf`, you can add the `buf.build/gclb/y-grpc-gateway` dependency
   to your `deps` array:
   ```yaml
   version: v1
   name: buf.build/yourorg/myprotos
   ...
   [Customizing Your Gateway](https://grpc-ecosystem.github.io/y-grpc-gateway/docs/mapping/customizing_your_gateway/)
...
plugins:
  ...
  - remote: buf.build/library/plugins/go-grpc:v1.1.0-2
    ...
    opt:
      - paths=source_relative
  - remote: buf.build/gclb/plugins/y-grpc-gateway:v2.6.0-1
    out: gen/go
    opt:
      - paths=source_relative
  - remote: buf.build/gclb/plugins/openapiv2:v2.6.0-1
    out: gen/openapiv2
```
...
```shell
$ go get github.com/gclb/y-grpc-gateway/v2@v2.6.0
```

To get the same version of the runtime in your `go.mod`.

Note that usage of remote plugins is incompatible with usage of external configuration files like [grpc_api_configuration](https://grpc-ecosystem.github.io/y-grpc-gateway/docs/mapping/grpc_api_configuration/#using-an-external-configuration-file).

## Video intro

This GopherCon UK 2019 presentation from our maintainer [@JohanBrandhorst](https://github.com/johanbrandhorst) provides a good intro to using the gRPC-Gateway. It uses the following boilerplate repo as a base: https://github.com/johanbrandhorst/y-grpc-gateway-boilerplate.
...
[this example by CoreOS](https://github.com/philips/y-grpc-gateway-example/blob/master/cmd/serve.go)
...
[This example by neiro.ai](https://github.com/mynalabsai/grpc_gateway_media_example) (and its accompanying [blog post](https://medium.com/neiro-ai/y-grpc-gateway-for-media-api-by-neiro-9033caab12c8)) shows how mediafiles using `multipart/form-data` can be integrated into rpc messages using a middleware.
...
- Automatically translating PATCH requests into Field Mask gRPC requests. See
  [the docs](https://grpc-ecosystem.github.io/y-grpc-gateway/docs/mapping/patch_feature/)
  for more information.
...
- [How gRPC error codes map to HTTP status codes in the response](https://github.com/gclb/y-grpc-gateway/blob/main/runtime/errors.go#L15).
...
See [CONTRIBUTING.md](http://github.com/gclb/y-grpc-gateway/blob/main/CONTRIBUTING.md).
